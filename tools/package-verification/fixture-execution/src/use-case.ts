...
class MemoryUseCaseExecutionStore implements UseCaseExecutionStore {
    ...
    async claim<TResult>(request: ClaimUseCaseExecutionRequest): Promise<ClaimUseCaseExecutionResult<TResult>> {
        ...
        if (existing && existing.intentId !== request.intent.id) return { type: 'intent-conflict', existingIntentId: existing.intentId };
        if (existing?.state === 'completed') return { type: 'completed', result: existing.result as TResult, completedAt: existing.completedAt ?? request.requestedAt };
        if (existing?.state === 'in-progress' && existing.lease) return { type: 'already-in-progress', lease: existing.lease };

        const version = (existing?.version ?? 0) + 1;
        const lease = { ownerId: request.leaseOwnerId, version, acquiredAt: request.requestedAt, expiresAt: new Date(Date.parse(request.requestedAt) + request.leaseDurationMs).toISOString() } as ExecutionLease;
        this.records.set(request.executionId, { intentId: request.intent.id, state: 'in-progress', lease, version });
    ...
    async complete<TResult>(request: CompleteUseCaseExecutionRequest<TResult>): Promise<CompleteUseCaseExecutionResult> {
        ...
        if (!record || record.state !== 'in-progress' || !record.lease) return { type: 'not-in-progress' };
        if (!sameLease(record.lease, request.lease)) return { type: 'lease-conflict' };
...
function sameLease(current: ExecutionLease, candidate: { readonly ownerId: ExecutionLease['ownerId']; readonly version: ExecutionLease['version'] }): boolean {
...
const executor = createUseCaseExecutor({
    dependencies: { clock, executionIdFactory: new DefaultExecutionIdFactory(), store },
