# EDP Observability Model

Status: approved design for Epic #134. This document defines the observability contract to be implemented by the Epic. It is not part of `docs/architecture/` until the corresponding runtime instrumentation exists.

## Purpose

EDP exposes typed lifecycle facts from its centralized execution boundaries:
...
| `execution.completed` | A started execution reached a terminal result | Are executions succeeding and how long do they take? | throughput; success/rejection/error/timeout rate; execution latency | operation; outcome; optional tenant; bounded failure type when applicable |
...
| `attempt.completed` | A handler attempt ends | Are attempts failing, timing out, or becoming slow before the whole execution ends? | attempt outcome rate; attempt latency; retryable failure rate | operation; outcome; retryable; optional tenant; bounded failure type when applicable |
...
| `guard.rejected` | A configured guard rejects execution | Which Operations are being stopped by guards? | guard rejection count/rate | operation; optional tenant; bounded guard identity only when explicitly provided by the guard contract |
| `rate-limit.rejected` | A configured rate limit rejects execution | Which Operations are being throttled? | rate-limit rejection count/rate | operation; optional tenant; bounded rate-limit scope only when explicitly modeled |
...
| `source.completed` | Reader executed the resolved source handler | How often do Reads reach the source and is the source slow/failing? | source execution rate; source error rate; source latency | read; outcome |
...
| `distributed-coordination.completed` | Distributed coordination made a claim/wait decision or failed | Is shared miss coordination effective and healthy? | owner/waiter rate; coordinator unavailable rate; ownership-lost rate; coordination latency | read; outcome (`owner`, `waiter`, `unavailable`, `ownership-lost`) |
...
If a failure cannot be classified into a dedicated bounded category, it remains `error` and may carry a bounded normalized failure type for telemetry consumers.

## UseCaseExecutor catalog

UseCaseExecutor currently has no stable UseCase name or tenant field in its public request contract. The observability model must not derive metric identity from `constructor.name` or inspect arbitrary extended context. Metrics therefore remain aggregate unless a future explicit bounded identity is added to the execution contract.

| Observation | Meaning | Operational question | Production signals | Metric dimensions |
| --- | --- | --- | --- | --- |
| `execution.requested` | UseCaseExecutor received an execution request | How much durable UseCase demand is arriving? | request rate | none beyond bounded deployment/resource attributes supplied outside EDP |
| `claim.completed` | Durable claim returned a result | Are invocations new, replayed, duplicated, or conflicting? | claimed/replayed/already-in-progress/intent-conflict count and rate; claim latency | outcome (`claimed`, `completed`, `already-in-progress`, `intent-conflict`) |
| `execution.started` | A successful claim entered UseCase execution | How many UseCases are actively executing under this Executor? | active executions | none |
| `execution.completed` | A started UseCase execution ended | Are UseCases succeeding and how long do they run? | success/error rate; execution latency | outcome (`success` or `error`) |
| `completion.completed` | Durable completion transition returned | Is successful UseCase work being durably committed? | completion success/rejection count; completion latency | outcome (`completed` or `rejected`) |
| `release.completed` | Best-effort release after a UseCase failure returned or failed | Are failed invocations being released cleanly for retry/recovery? | release success/failure count; release latency | outcome (`released` or `error`) |
...
A telemetry consumer should be able to build at least the following operational views without adding new instrumentation points:

| Boundary | Required production view |
| --- | --- |
...
| UseCaseExecutor | request rate, active executions, p50/p95/p99 execution latency, success/error rate, completed-result replay rate, duplicate/conflict rate, durable completion health, release health |

## Contract boundary

The implementation introduced by this Epic must preserve three separate lifecycle models:

- `RunnerObservation`;
- `ReaderObservation`;
- `UseCaseExecutorObservation`.

They may share a generic one-method observer shape and safety utilities, but they must not be collapsed into one untyped or loosely typed universal event model.
