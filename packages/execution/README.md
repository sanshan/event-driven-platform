...
`ExecutionFailureError` is a concrete `Error` subclass that carries an `ExecutionFailure` as `executionFailure` and forwards the standard `Error.cause`. Runner throws it (or a subclass of it) for every failure it can classify; Reader and UseCaseExecutor are being retrofitted onto it as well, so that a consumer will eventually be able to catch failures from all three execution boundaries with a single `instanceof ExecutionFailureError` check — for example in a Nest exception filter that maps `executionFailure.code` to an HTTP response. EDP itself does not perform that mapping; it only makes the failure catchable.
